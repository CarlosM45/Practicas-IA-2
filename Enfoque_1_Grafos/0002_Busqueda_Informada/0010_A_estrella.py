...
# Algoritmo A*, que busca el camino más corto en grafos ponderados con heurística
...
# Algoritmo AO*
def ao_estrella(problema):
    raiz = crea_nodo_raiz(problema)
    grafo = {raiz: []}  # Representación del grafo AND-OR
    solucion = set()  # Nodos que forman parte de la solución
    pendientes = [raiz]  # Nodos pendientes de expansión
    ...
    while pendientes:
        ...
        if nodo in grafo and grafo[nodo]:  # Si ya fue expandido
            continue
        ...
        for nombre_accion in nodo.acciones.keys():
            accion = Accion(nombre_accion)
            hijo = crea_nodo_hijo(problema, nodo, accion, agregar=False)
            hijos.append(hijo)
        ...
        actualizar_valores(grafo, nodo, solucion)

    return construir_solucion(raiz, grafo, solucion)


# Función para actualizar los valores de los nodos en el grafo
def actualizar_valores(grafo, nodo, solucion):
    ...
    for hijo in grafo[nodo]:
        if hijo in solucion:
            valores_hijos.append(hijo.valores)
        else:
            valores_hijos.append(float('inf'))  # Nodo no resuelto

    # Seleccionar el mejor conjunto de hijos
    nodo.valores = min(valores_hijos)


# Función para construir la solución a partir del grafo
def construir_solucion(raiz, grafo, solucion):
    camino = []
    nodo = raiz
    while nodo in grafo and grafo[nodo]:
        camino.append(nodo)
        hijos = grafo[nodo]
        nodo = min(hijos, key=lambda hijo: hijo.valores)
    camino.append(nodo)
    return camino
...
solucion_a = a_estrella(problema_resolver)
solucion_ao = ao_estrella(problema_resolver)
print("Solución A*")
muestra_solucion(solucion_a)
print("\nSolución AO*")
muestra_solucion(solucion_ao)
