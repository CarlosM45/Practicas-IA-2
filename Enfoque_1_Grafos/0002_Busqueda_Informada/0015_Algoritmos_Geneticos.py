...
def fitness_function(params):
    a, b, c = params
    if a <= 0:
        return -float('inf')  # Penalizar gravemente curvas que abran hacia abajo
    vertex_x = -b / (2 * a) # Valor x de vertex
...
def mutation(individual, mutation_rate, lower_bound, upper_bound):
    ...
    for i in range(len(individual)):
        if random.random() < mutation_rate:
            ...
            # Asegurarse de que el individuo está dentro de los límites
...
def genetic_algorithm(population_size, lower_bound, upper_bound, generations, mutation_rate):
    ...
    for generation in range(generations):
        ...
        # Reemplazar la población anterior con la nueva, preservando al mejor individuo
    ...
    # Plot la población de una generación
    ...
    # Plot valores de a, b y c por generación
    ...
    # Plot valores fitness de cada generación
    ...
    # Plot función cuadrática de cada generación
    fig, ax = plt.subplots()
    ...
    # Crear un subplot para la colorbar
    cax = fig.add_axes([0.92, 0.2, 0.02, 0.6])  # [izquierda, fondo, ancho, altura]
